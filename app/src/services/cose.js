...
function decodeCOSESign1(buf) {
  ...
  if (!ArrayBuffer.isView(protectedHeader)) {
    throw new Error(
      'Not a COSE_Sign1 message: protected header not wrapped in bstr'
    )
  }
  ...
  if (
    almost.protectedHeader.ctyp &&
    almost.protectedHeader.ctyp.startsWith('application/credential+json')
  ) {
